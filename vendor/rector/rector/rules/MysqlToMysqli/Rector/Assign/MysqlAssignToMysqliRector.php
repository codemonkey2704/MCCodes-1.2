...
final class MysqlAssignToMysqliRector extends AbstractRector
{
     ...
     * @var string
     */
    private const mysql_DATA_SEEK = 'mysql_data_seek';
    /**
     * @readonly
...
CODE_SAMPLE
    ...
    {
        $funcCall->name = new Name(self::mysql_DATA_SEEK);
    ...
    {
        $funcCall->name = new Name('mysql_select_db');
        $newAssignNode = new Assign($assign->var, new FuncCall(new Name('mysql_query'), [$funcCall->args[1]]));
        $this->nodesToAddCollector->addNodeAfterNode($newAssignNode, $assign);
    ...
    {
        $funcCall->name = isset($funcCall->args[1]) ? new Name('mysql_fetch_field_direct') : new Name('mysql_fetch_field');
        return $assign;
    }
    ...
    private function processFieldToFieldDirect(Assign $assign, FuncCall $funcCall) : ?Assign
    {
        foreach (self::FIELD_TO_FIELD_DIRECT as $funcName => $property) {
            if ($this->isName($funcCall, $funcName)) {
                ...
                $funcCall->name = new Name('mysql_fetch_field_direct');
